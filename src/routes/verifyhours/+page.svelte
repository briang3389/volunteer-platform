				...
				{#each volunteers as volunteer}
					<tr>
						...
						<td class="px-5 py-5 border-b border-gray-200 bg-white text-sm flex">
							<button
								class="text-white bg-green-500 hover:bg-green-700 rounded py-1 px-3 mr-2"
								on:click={() => handleAccept(volunteer.eventid, volunteer.id)}
							>
								Accept
							</button>
							<button
								class="text-white bg-red-500 hover:bg-red-700 rounded py-1 px-3"
								on:click={() => handleDecline(volunteer.eventid, volunteer.id)}
							>
								Decline
							</button>
						</td>
